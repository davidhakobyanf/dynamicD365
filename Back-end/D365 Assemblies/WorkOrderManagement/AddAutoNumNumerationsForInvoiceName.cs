...
{
    public class AddAutoNumNumerationsForInvoiceName : IPlugin
    {
        public void Execute(IServiceProvider serviceProvider)
        {
            ...
            try
            {
                if (context.InputParameters.Contains("Target") && context.InputParameters["Target"] is Entity)
                {
                    ...
                    if (customerAsset.Contains("cr651_fk_customer") && customerAsset["cr651_fk_customer"] is EntityReference)
                    {
                        EntityReference accountReference = (EntityReference)customerAsset["cr651_fk_customer"];
                        ...
                        if (account != null && account.Contains("cr651_name"))
                        {
                            ...
                            QueryExpression query = new QueryExpression("cr651_invoice")
                            {
                                ...
                                Criteria =
                                {
                                    Conditions =
                                    {
                                        new ConditionExpression("cr651_fk_customer", ConditionOperator.Equal, accountReference.Id)
                                    }
                                }
                            };
                    ...
                    {
                        tracingService.Trace("cr651_fk_customer not found on the target entity.");
                    }
                }
                else
                {
